...
import { ColumnDataType, AsignacionDocenteModel, SearchFuncionarioDto, DocenteModel } from '../../domains';
...
const Docentes: React.FC = () => {
  ...
  const [data, setData] = useState<DocenteModel[]>([]);
  ...
  type DataIndex = keyof ColumnDataType<DocenteModel>["data"];
  ...
  const handleButtonClick = async () => {
    setLoading(true);
    const url = `${BASE_URL}/api/v1/scraper/obtener/docentes/`;
    try {
      let payload: {
      ...
      const response = await axios.post<AxiosResponse<DocenteModel[]>>(url, payload);
      setLoading(false);
      if (response.data.data != null || response.data.data != undefined) {
        for (let d of response.data.data) {
          Object.assign(d, { key: d.id })
          for(let c of d.detalles || []) {
  ...
  const getColumnSearchProps = (dataIndex: DataIndex): ColumnType<ColumnDataType<DocenteModel>["data"]> => ({
  ...
  const getColumnSelectProps = (dataIndex: DataIndex, tipo: string = "year"): ColumnType<ColumnDataType<DocenteModel>["data"]> => ({
    filterDropdown: () => (
      tipo == "year" ?
        <div style={{ padding: 8 }} onKeyDown={(e) => e.stopPropagation()}>
          <Select
  ...
  let columnas: ColumnsType<ColumnDataType<DocenteModel>["data"]> = [];
  let obj: DocenteModel = new DocenteModel()
  obj.getAtributos(["id", "created_at", "updated_at", "deleted_at", "detalles"]).forEach((key, index) => {
    let k: any = key;
    columnas.push({
      title: key,
  ...
  const expandedRowRender = (record: DocenteModel) => {
    const columnsAsignaciones: TableColumnsType<ColumnDataType<AsignacionDocenteModel>["data"]> = [];
    let obj: AsignacionDocenteModel = new AsignacionDocenteModel()
    obj.getAtributos(["created_at", "updated_at", "deleted_at", "id"]).forEach((key) => {
    ...
    return <Table columns={columnsAsignaciones} dataSource={record?.detalles} pagination={false} />;
  };
  ...
  return (
    <Content style={{ margin: '0 16px' }}>
      <Breadcrumb style={{ margin: '16px 0' }}>
        ...
        <Breadcrumb.Separator>Docentes</Breadcrumb.Separator>
      </Breadcrumb>
      <Table
        columns={columnas}
        dataSource={data}
